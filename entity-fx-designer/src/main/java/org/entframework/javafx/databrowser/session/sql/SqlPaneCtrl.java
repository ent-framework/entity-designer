...
import org.entframework.javafx.databrowser.services.progress.ProgressUtil;
...
import org.entframework.javafx.databrowser.session.completion.SQLTextAreaTextComponentAdapter;
import org.entframework.javafx.databrowser.session.sql.bookmark.BookmarkManager;
import org.entframework.javafx.databrowser.session.sql.features.DuplicateLineCommand;
import org.entframework.javafx.databrowser.session.sql.features.EscapeDateCtrl;
import org.entframework.javafx.databrowser.session.sql.features.SchemaUpdater;
import org.entframework.javafx.databrowser.session.sql.features.SqlToTableCtrl;
import org.entframework.javafx.databrowser.session.sql.searchchandreplace.ReplaceCtrl;
import org.entframework.javafx.databrowser.session.sql.searchchandreplace.SearchCtrl;
...
public class SqlPaneCtrl {
    ...
    private void onGoOn(StatementExecution statExec, String sql, SQLCancelTabCtrl sqlCancelTabCtrl, SqlExecutionFinishedListener sqlExecutionFinishedListener, PredecessorTabData predecessorTabData) {
        ...
        if (null != statExec.getFirstSqlException()) {
            String errMsg = MessageHandler.errorSQLNoStack(statExec.getFirstSqlException());
            _mh.error(errMsg);
            ...
            for (int i = 0; i < statExec.getQueryResults().size(); i++) {
                ...
                ResultTabCtrl resultTabCtrl;

                if (0 == i) {
                    sqlCancelTabCtrl.convertToInfoTab(statExec.getCompleteTime());
                    resultTabCtrl = new ResultTabCtrl(_sessionTabContext.getSession(), sqlResult, sql, sqlCancelTabCtrl, getTableStateOrNull(predecessorTabData));
                } else {
                    resultTabCtrl = new ResultTabCtrl(_sessionTabContext.getSession(), sqlResult, sql, null, getTableStateOrNull(predecessorTabData));
                }

                Tab outputTab = resultTabCtrl.getTab();
