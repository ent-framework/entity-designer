...
public class SpringUtil implements BeanFactoryPostProcessor, ApplicationContextAware {
    ...
    public static <T> T getBeanWithGenericType(Class<T> supper, Type param) {
        Map<String, T> beans = getBeanFactory().getBeansOfType(supper);
        Collection<T> values = beans.values();
        for (T bean : values) {
            ResolvableType resolvableType = ResolvableType.forClass(supper, bean.getClass());
            ResolvableType typeGeneric = resolvableType.getGeneric(0);
            if (typeGeneric.getType().equals(param)) {
                return bean;
            }
