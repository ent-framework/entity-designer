...
public abstract class AbstractFxmlView implements ApplicationContextAware {
    ...
    private FXMLLoader loadSynchronously(final URL resource, final Optional<ResourceBundle> bundle) throws IllegalStateException {
        ...
        loader.setCharset(getResourceBundleCharset());
    ...
    protected void initFirstView() {
        ...
        Scene scene = getView().getScene() != null ? getView().getScene() : new Scene(getView(), 800, 600);
    ...
    public void showViewAndWait(Window window, Modality modality, String title) {
        showViewAndWait(window, modality, title, false, null);
    }
     ...
     * @param window   The owner of the FxmlView instance
     * @param modality See {@code javafx.stage.Modality}.
     * @param title    See {@code javafx.stage.Modality}.
     */
    public void showViewAndWait(Window window, Modality modality, String title, boolean savePosition, String prefix) {
        ...
        if (stage == null || currentStageModality != modality || !Objects.equals(stage.getOwner(), window)) {
            stage = createStage(modality);
            if (savePosition) {
                final Pref pref = new Pref(getClass());
                final StageDimensionSaver dimensionSaver = new StageDimensionSaver(prefix, stage, pref, 1000d, 800d, window);
                stage.setOnCloseRequest(windowEvent -> dimensionSaver.save());
            }
